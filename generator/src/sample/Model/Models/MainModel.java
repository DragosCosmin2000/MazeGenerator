...
import sample.Model.Algorithms.AldousBroderAlgorithm;
import sample.Model.Algorithms.HuntAndKillAlgorithm;
...
public class MainModel extends Task<Long> {
    ...
    public void configureModel(Settings settings, Pane mazePane) {
        ...
        // The solver.
        if(this.settings.getAlgorithmName().equals("Recursive Backtracker Algorithm")) {
            solver = new RecursiveBacktrackerAlgorithm(pb);
        }
        else
            if(this.settings.getAlgorithmName().equals("Aldous-Broder algorithm"))
                solver=new AldousBroderAlgorithm(pb);
            else
                solver=new HuntAndKillAlgorithm(pb);
    }
